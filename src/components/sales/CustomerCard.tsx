              ...
              <button
                onClick={() => {
                  ...
                  const sections = buildCustomerSections(
                    { ...d, company: d.company || company, assignee: d.assignee || customer.sales_user_name },
                    { name, phone, company }
                  )
                  if (d.contract_fee) {
                    sections.push({
                      title: '계약 정보', color: '#059669',
                      fields: [
                        { label: '계약금',   value: d.contract_fee },
                        { label: '입금액',   value: d.payment_amount },
                        { label: '미입금',   value: d.unpaid_amount },
                        { label: '수수료율', value: d.commission_rate },
                      ],
                    })
                  }
                  printIncall({
                    companyName: company || '(업체명 없음)',
                    salesName:   d.assignee || customer.sales_user_name || '',
                    sections,
                    callResult:    d.call_result    || '',
                    closingResult: d.closing_result || '',
                    subcallDate:   d.subcall_date   || '',
                    timeline: callTimeline.map(e => ({ time: e.time, author: e.author, content: e.content })),
                    notes:     notes || '',
                    asRequest: d.as_request || '',
                  })
                }}
