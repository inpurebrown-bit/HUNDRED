...
function ContractModal({ company, cumulativeBase, initialMemo = '', initialNoRefund = false, onClose, onConfirm }: ContractModalProps) {
  ...
  async function handleConfirm() {
    ...
    await onConfirm({
      ...
      // 카드결제는 자동신고 → 무조건 발급 / 아니면 부가세 포함 여부로 결정
      tax_invoice:         paymentMethod === '카드' || vatIncluded ? '발급' : '미발급',
          ...
          <div className="sticky top-0 flex flex-wrap items-center gap-1.5 px-4 py-2.5 border-b border-gray-100 bg-white z-10">
            ...
            <button
              ...
              onClick={e => {
                ...
                printIncall({
                  companyName: d.company || (c as any).company || c.name || '',
                  salesName:   d.sales_user_name || (c as any).sales_user_name || '',
                  sections:    buildCustomerSections(d, { name: c.name, phone: c.phone, company: (c as any).company }),
                  callResult:    d.call_result    || '',
                  closingResult: d.closing_result || '',
                  subcallDate:   d.subcall_date   || '',
                  timeline: c.call_timeline || [],
                })
              }}
